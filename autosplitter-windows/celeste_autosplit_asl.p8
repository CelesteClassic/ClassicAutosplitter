...
function title_screen()
  printh('-- new run --')
  poke(0x5f80, 0) -- 1st GPIO pin, for current level
  got_fruit = {}
    for i=0,29 do
...
function next_room()
 ...
 elseif room.x==5 and room.y==3 then
  ...
  poke(0x5f80,room.x%8+room.y*8+1)

end

function two_digit_str(x,c) -- Code from Cominixo with modifications by me
...
function load_room(x,y)
  ...
  if lvl<=31 and lvl>prev_room then
    cmd = 185
  ...
  if cmd == 185 then
    local cs=flr(100*frames/30+0.5)
    local timestr = two_digit_str(minutes)..':'..two_digit_str(seconds)..'.'..two_digit_str(cs)
    printh((lvl-1).."00m - "..timestr)
    for i=1,#timestr do
      poke(0x5f84+i, ord(sub(timestr,i,i)))
    end
    poke(0x4303+#timestr, 0x0a)
    serial(0x809,0x5f84, #timestr+3)
  end
...
function _update()
  ...
  if frames==0 and level_index()<30 then
    ...
    local timestr = two_digit_str(minutes)..':'..two_digit_str(seconds)..'.00'
    for i=1,#timestr do
      poke(0x5f84+i, ord(sub(timestr,i,i)))
    end
    poke(0x5f85+#timestr, 0x0a)
    serial(0x809,0x5f84, #timestr+2)
  end
